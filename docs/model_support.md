...
| Wall Qwen3.5 exact flow policy | `rpu_backend.api.WallQwen35Policy` | Source-only | Controlled evaluation of the exact locally admitted checkpoint only: FP16, batch 1, robot ID `10070`, normalizer `x2_normal`, fixed mask `[1]*20+[0]*6`, exactly three canonical cameras with Dataset-V2 single-stage BICUBIC preprocessing, initial multimodal prefix `<=384`, cache-only base decode with no language `lm_head`, action output `[1,32,26]`, 10 Euler steps, and the cold multi-handle setting `RPU_FUSED_COEXIST_KEEP_PERSISTENT_GEN=1`. The Qwen3.5 vision path is numeric-blocked and requires explicit opt-in; release numerical, independent Graph-lifecycle, and task gates remain pending |
...
| Wall Qwen3.5 exact flow policy | Exact checkpoint/profile admission and the initial FP16 batch-1 input envelope are enforced. The open-loop READY probe warms one request, records one identical repeat, and reports component lifecycle admission. Action proves stable replay for the repeated prefix, but the two Vision geometries contend for a one-entry cache and base Prefill remains a bounded one-shot; full READY and the independent release lifecycle gate remain pending | On-board same-dtype parity and FP32-anchor validation remain pending | The runner emits aligned absolute-action metrics, but no frozen acceptance threshold or parity-matched RNG reference is bound; representative release task validation remains pending |
