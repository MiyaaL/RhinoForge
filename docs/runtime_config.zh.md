...
| `RPU_QWEN35_WALL_PREFIX_COPY_ONCE` | `0`；精确 `1` 启用 | Wall Qwen3.5 action request / **CALL**；新进程 | 仅诊断 arm：每个 public request 只复制一次六层物理 K/V prefix，十个 suffix step 复用独立的 action-owned prefix。它依赖固定 Wall suffix-write 契约，必须通过 same-noise parity 与 cache 完整性校验。 |
