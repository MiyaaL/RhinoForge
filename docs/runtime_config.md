...
| `QWEN3_5_VISION_GRAPH_MAX_ENTRIES` | `1`; positive integer | Vision Graph construction / **MODEL** | Bounds retained Vision signatures. Wall defaults to `1`: one packed graph covers the three images, keyed by their ordered patch lengths. Larger values increase persistent queue/Graph memory and require a fresh model. |
...
| `RPU_QWEN35_WALL_GEMM_TILES` | Unset; comma/semicolon-separated `MxNxK=n_tile` entries, where `n_tile` is one of `128,112,96,80,64,48,32` | Wall Qwen3.5 action Graph construction / **BUILD**; fresh process | Board-free/board A/B only. Overrides the generated FP16 ACC32 tile for an exact local shape; unknown shapes retain auto-tiling. It is cold and diagnostic-only, and does not add a GEMM epilogue asset. |
